...
class ViewCie10a9 extends SugarView {

    function ViewCie10a9() {
        parent::SugarView();
    }
    ...
    function display() {
        ...
        if (!empty($_REQUEST['export_trigger'])) {
            ...
            $field_headings = array('DIAGNOSTICO PRINCIPAL MAS COMUN ANEXO 9', 'DESCRIPCION PRINCIPAL', 'CANTIDAD');
            ...
            $query = "SELECT
            diag.name as 'DIAGNOSTICO PRINCIPAL MAS COMUN ANEXO 9',
            REPLACE(REPLACE(REPLACE(REPLACE(diag.description,char(09),''),char(10),''),char(11),''),char(13),'') as 'DESCRIPCION PRINCIPAL',
            count(cie_cie10_id_c) as 'CANTIDAD'

        FROM
            gbi_anexo9_cstm a9
            LEFT JOIN cie_cie10 diag ON diag.id = a9.cie_cie10_id_c
            INNER JOIN gbi_anexo9 aa9 on aa9.id = a9.id_c
            where
            aa9.deleted = '0'";
            ...
            if (!empty($_REQUEST["fecha_inicio"]) && !empty($_REQUEST["fecha_final"]))
            {
                ...
                $query .= "AND DATE_FORMAT(DATE_SUB(aa9.date_entered,INTERVAL + 5 HOUR),'%Y/%m/%d') BETWEEN '".$_REQUEST["fecha_inicio"]."' AND '".$_REQUEST["fecha_final"]."'";
            }
            ...
            $filename = 'DIAGNOSTICO_CIE10_A9_MASCOMUNES_'.date("dmY").".csv";
        ...
        else
        {
            ...
            ?>
            <h1>Reporte - Diagnóticos CIE 10 más comunes Anexo 9</h1>
                    ...
                    <tr>
                        <td class="tabForm">
                            ...
                            <form id="reportfm" name="reportfm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" onsubmit="return validateForm()">
                                ...
                                <input type="hidden" name="action" value="cie10a9" />
                                </br>&nbsp;
