...
class ViewCie_A2 extends SugarView {
    ...
    function display() {
        ...
        if (!empty($_REQUEST['export_trigger'])) {
            ...
            $field_headings = array('DIAGNOSTICO ANEXO 2', 'DESCRIPCION PRINCIPAL', 'CANTIDAD');
            ...
            $query = "SELECT 
            diag.name AS 'DIAGNOSTICO PRINCIPAL MAS COMUN ANEXO 2',
            REPLACE(REPLACE(REPLACE(REPLACE(diag.description, CHAR(09), ''),
                        CHAR(10),
                        ''),
                    CHAR(11),
                    ''),
                CHAR(13),
                '') AS 'DESCRIPCION PRINCIPAL',
            COUNT(cie_cie10_id3_c) AS 'CANTIDAD'
        FROM
            gbian_anexo2_cstm a2
                INNER JOIN
            gbian_anexo2 aa2 ON aa2.id = a2.id_c
                LEFT JOIN
            cie_cie10 diag ON diag.id = a2.cie_cie10_id3_c
        WHERE
            aa2.deleted = '0'";
            ...
            if (!empty($_REQUEST["fecha_inicio"]) && !empty($_REQUEST["fecha_final"]))
            {
                ...
                $query .= "AND DATE_FORMAT(DATE_SUB(a2.fecha_ingreso_c,INTERVAL + 5 HOUR),'%Y/%m/%d') BETWEEN '".$_REQUEST["fecha_inicio"]."' AND '".$_REQUEST["fecha_final"]."'";
            }

            $query .= "group by a2.cie_cie10_id3_c having count(cie_cie10_id3_c) > 1 order by count(cie_cie10_id3_c) desc limit 5";
            ...
            $filename = 'DIAGNOSTICO_CIE10_A2_MASCOMUNES_'.date("dmY").".csv";
        ...
        else
        {
            ...
            ?>
            <h1>Reporte - Diagnóticos CIE 10 más comunes Anexo 2</h1>
                    ...
                    <tr>
                        <td class="tabForm">
                            ...
                            <form id="reportfm" name="reportfm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" onsubmit="return validateForm()">
                                ...
                                <input type="hidden" name="action" value="cie_a2" />
                                </br>&nbsp;
