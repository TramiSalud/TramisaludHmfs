...
class ViewEnvio_A9 extends SugarView {

	function ViewEnvio_A9() {
 		parent::SugarView();
	}
...
//file_put_contents('\\\192.168.0.15\\htdocs\\tramisalud_dev_gbi\\modules\\GBI_Exports\\views\\log.txt', var_export($gridline, true), FILE_APPEND);		
		...
		if (!empty($_REQUEST['export_trigger'])) {
			...
			$query = "SELECT a9.name AS 'NRO. SOLICITUD', 
					CASE
						WHEN a9_c.correo_enviado_c = 0 THEN 'NO'
						WHEN a9_c.correo_enviado_c = 1 THEN 'SI'
					END AS ENVIADO,
CONCAT(
	users.first_name,
	' ',
	users.last_name
					...
					FROM gbi_anexo9 a9
					LEFT JOIN gbi_anexo9_cstm a9_c ON a9_c.id_c = a9.id
					LEFT JOIN users ON users.id = a9.created_by
					WHERE a9.deleted = 0 ";
			
			if (!empty($_REQUEST["fecha_inicio"]) && !empty($_REQUEST["fecha_final"]))
			{
				$query .= "AND a9.date_entered BETWEEN '".$_REQUEST["fecha_inicio"]."' AND DATE_ADD('".$_REQUEST["fecha_final"]."',INTERVAL 1 DAY) ";
			}
			
			$query .= "ORDER BY a9.date_entered ASC";
			...
			$filename = 'ENVIOS_SOLICITUDES_ANEXO_9_'.date("dmY").".csv";
		...
		else 
		{
			...
			<h1>EXPORTAR - ENVIO SOLICITUDES ANEXO 9</h1>
					...
					<tr>
						<td class="tabForm">
							<form name=reportfm action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
								...
								<input type="hidden" name="action" value="envio_a9" /> 
								</br>
