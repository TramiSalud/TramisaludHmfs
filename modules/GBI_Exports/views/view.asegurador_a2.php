...
class ViewAsegurador_A2 extends SugarView {

    function ViewAsegurador_A2() {
        parent::SugarView();
    }
    ...
    function display() {
        ...
        if (!empty($_REQUEST['export_trigger'])) {
            ...
            $field_headings = array('ASEGURADORES MAS GESTIONADOS', 'ANEXOS 2 CANTIDAD DE ANEXOS GESTIONADOS');
            ...
            COUNT(eps_eps_id_c) AS 'ANEXOS 2 CANTIDAD DE ANEXOS GESTIONADOS'
        FROM
            gbian_anexo2 a2
                LEFT JOIN
            eps_eps eps ON eps.id = a2.eps_eps_id_c
        WHERE
            a2.deleted = '0'";
            ...
            if (!empty($_REQUEST["fecha_inicio"]) && !empty($_REQUEST["fecha_final"]))
            {
                ...
                $query .= "AND DATE_FORMAT(DATE_SUB(a2.date_entered,INTERVAL + 5 HOUR),'%Y/%m/%d') BETWEEN '".$_REQUEST["fecha_inicio"]."' AND '".$_REQUEST["fecha_final"]."'";
            }

            $query .= "group by a2.eps_eps_id_c having count(eps_eps_id_c) > 1 order by count(eps_eps_id_c) desc limit 5";
            ...
            $filename = 'ASEGURADORES_A2_MASCOMUNES_'.date("dmY").".csv";
        ...
        else
        {
            ...
            ?>
            <h1>Reporte - Asegurador Más Gestionado Anexo 2</h1>
                    ...
                    <tr>
                        <td class="tabForm">
                            ...
                            <form id="reportfm" name="reportfm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" onsubmit="return validateForm()">
                                ...
                                <input type="hidden" name="action" value="asegurador_a2" />
                                </br>&nbsp;
