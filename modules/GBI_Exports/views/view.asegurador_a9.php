...
class ViewAsegurador_A9 extends SugarView {

    function ViewAsegurador_A9() {
        parent::SugarView();
    }
    ...
    function display() {
        ...
        if (!empty($_REQUEST['export_trigger'])) {
            ...
            $field_headings = array('ASEGURADORES MAS GESTIONADOS', 'ANEXOS 9 CANTIDAD DE ANEXOS GESTIONADOS');
            ...
            COUNT(eps_eps_id_c) AS 'ANEXOS 9 CANTIDAD DE ANEXOS GESTIONADOS'
        FROM
            gbi_anexo9 a9
                LEFT JOIN
            eps_eps eps ON eps.id = a9.eps_eps_id_c
        WHERE
            a9.deleted = '0'";
            ...
            $query .= "group by a9.eps_eps_id_c having count(eps_eps_id_c) > 1 order by count(eps_eps_id_c) desc limit 5";
            ...
            $filename = 'ASEGURADORES_A9_MASCOMUNES_'.date("dmY").".csv";
        ...
        else
        {
            ...
            ?>
            <h1>Reporte - Asegurador Más Gestionado Anexo 9</h1>
                    ...
                    <tr>
                        <td class="tabForm">
                            ...
                            <form id="reportfm" name="reportfm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" onsubmit="return validateForm()">
                                ...
                                <input type="hidden" name="action" value="asegurador_a9" />
                                </br>&nbsp;
