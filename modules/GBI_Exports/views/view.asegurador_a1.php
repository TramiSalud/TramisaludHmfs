...
class ViewAsegurador_A1 extends SugarView {

    function ViewAsegurador_A1() {
        parent::SugarView();
    }
    ...
    function display() {
        ...
        if (!empty($_REQUEST['export_trigger'])) {
            ...
            $field_headings = array('ASEGURADORES MAS GESTIONADOS', 'ANEXOS 1 CANTIDAD DE ANEXOS GESTIONADOSL');
            ...
            COUNT(eps_eps_id_c) AS 'ANEXOS 1 CANTIDAD DE ANEXOS GESTIONADOS'
        FROM
            basa1_anexo1 a1
                LEFT JOIN
            eps_eps eps ON eps.id = a1.eps_eps_id_c
        WHERE
            a1.deleted = '0'";
            ...
            if (!empty($_REQUEST["fecha_inicio"]) && !empty($_REQUEST["fecha_final"]))
            {
                ...
                $query .= "AND DATE_FORMAT(DATE_SUB(a1.date_entered,INTERVAL + 5 HOUR),'%Y/%m/%d') BETWEEN '".$_REQUEST["fecha_inicio"]."' AND '".$_REQUEST["fecha_final"]."'";
            }

            $query .= "GROUP BY a1.eps_eps_id_c HAVING COUNT(eps_eps_id_c) > 1 ORDER BY COUNT(eps_eps_id_c) DESC LIMIT 5";
            ...
            $filename = 'ASEGURADORES_A1_MASCOMUNES_'.date("dmY").".csv";
        ...
        else
        {
            ...
            ?>
            <h1>Reporte - Asegurador Más Gestionado Anexo 1</h1>
                    ...
                    <tr>
                        <td class="tabForm">
                            ...
                            <form id="reportfm" name="reportfm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" onsubmit="return validateForm()">
                                ...
                                <input type="hidden" name="action" value="asegurador_a1" />
                                </br>&nbsp;
