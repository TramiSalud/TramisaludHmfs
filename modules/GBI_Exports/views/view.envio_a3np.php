...
class ViewEnvio_A3NP extends SugarView {

	function ViewEnvio_A3NP() {
 		parent::SugarView();
	}
...
//file_put_contents('\\\192.168.0.15\\htdocs\\tramisalud_dev_gbi\\modules\\GBI_Exports\\views\\log.txt', var_export($gridline, true), FILE_APPEND);		
		...
		if (!empty($_REQUEST['export_trigger'])) {
			...
			$query = "SELECT a3np.name AS 'NRO. SOLICITUD', 
					CASE
						WHEN a3np_c.correo_enviado_c = 0 THEN 'NO'
						WHEN a3np_c.correo_enviado_c = 1 THEN 'SI'
					END AS ENVIADO,
 					CONCAT(
					users.first_name,
					' ',
					...
					FROM gbi_anexo3npos a3np
					LEFT JOIN gbi_anexo3npos_cstm a3np_c ON a3np_c.id_c = a3np.id
					LEFT JOIN users ON users.id = a3np.created_by
					WHERE a3np.deleted = 0  ";
			
			if (!empty($_REQUEST["fecha_inicio"]) && !empty($_REQUEST["fecha_final"]))
			{
				$query .= "AND a3np.date_entered BETWEEN '".$_REQUEST["fecha_inicio"]."' AND DATE_ADD('".$_REQUEST["fecha_final"]."',INTERVAL 1 DAY) ";
			}
			
			$query .= "ORDER BY a3np.date_entered ASC";
			...
			$filename = 'ENVIOS_SOLICITUDES_ANEXO_3NP_'.date("dmY").".csv";
		...
		else 
		{
			...
			<h1>EXPORTAR - ENVIO SOLICITUDES ANEXO 3 NO POS</h1>
					...
					<tr>
						<td class="tabForm">
							<form name=reportfm action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
								...
								<input type="hidden" name="action" value="envio_a3np" /> 
								</br>
