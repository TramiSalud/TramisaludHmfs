...
class ViewSolicitudes extends SugarView {
...
//file_put_contents('\\\192.168.0.15\\htdocs\\tramisalud_dev_gbi\\modules\\GBI_Exports\\views\\log.txt', var_export($gridline, true), FILE_APPEND);		
		...
		if (!empty($_REQUEST['export_trigger'])) {
			...
			$field_headings = array('EPS', 'PRIORIDAD', 'TIPO_DOCUMENTO', 'DOCUMENTO', 'CODIGO_SERVICIO', 'CANTIDAD', 'FECHA_REMISION', 'CODIGO_ESPECIALIDAD', 'CODIGO_DIAGNOSTICO', 'OBSERVACIONES', 'CREADO POR');
			...
			$query = "SELECT eps.`name` AS EPS,
					CASE
						WHEN a3.prioridadatencion2 = 'prioritaria' THEN 'SI'
						WHEN a3.prioridadatencion2 = 'no_prioritaria' THEN 'NO'
					END AS PRIORIDAD,
					pc.tipo_documento_c AS TIPO_DOCUMENTO,
					p.name AS DOCUMENTO,
					CUP.`name` AS CODIGO_SERVICIO,
					...
					CONCAT(REPLACE(REPLACE(REPLACE(REPLACE(a3.justificacionclinica , CHAR(10), ''), CHAR(13), ''), CHAR(9), ''), CHAR(44), ''), '^^',
							CUP.description
					) AS OBSERVACIONES,
	CONCAT(
	users.first_name,
	...
	users.last_name
			...
			$filename = 'SOLICITUDES_ANEXO_3_'.date("dmY").".csv";
		...
		else 
		{
			...
			<h1>EXPORTAR SOLICITUDES ANEXO 3</h1>
