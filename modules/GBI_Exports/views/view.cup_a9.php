...
class ViewCup_A9 extends SugarView {
    ...
    function display() {
        ...
        if (!empty($_REQUEST['export_trigger'])) {
            ...
            $field_headings = array('CUP MAS RELEVANTE ANEXO 9', 'DESCRIPCION DE CUP', 'CANTIDAD DE VECES GESTIONADO');
            ...
            $query = "SELECT 
            cups.name AS 'CUP MAS RELEVANTE ANEXO 9',
            REPLACE(REPLACE(REPLACE(REPLACE(cups.description, CHAR(09), ''),
                        CHAR(10),
                        ''),
                    CHAR(11),
                    ''),
            ...
            COUNT(gbicu_cups_id_c) AS 'CANTIDAD DE VECES GESTIONADO'
        FROM
            gbi_anexo9 a9
                LEFT JOIN
            gbicu_cups cups ON cups.id = a9.gbicu_cups_id_c
        WHERE
            a9.deleted = '0'";
            ...
            if (!empty($_REQUEST["fecha_inicio"]) && !empty($_REQUEST["fecha_final"]))
            {
                ...
                $query .= "AND DATE_FORMAT(DATE_SUB(a9.date_entered,INTERVAL + 5 HOUR),'%Y/%m/%d') BETWEEN '".$_REQUEST["fecha_inicio"]."' AND '".$_REQUEST["fecha_final"]."'";
            }

            $query .= "group by a9.gbicu_cups_id_c having count(gbicu_cups_id_c) > 1 order by count(gbicu_cups_id_c) desc limit 5";
            ...
            $filename = 'CUP_A9_MASCOMUNES_'.date("dmY").".csv";
        ...
        else
        {
            ...
            ?>
            <h1>Reporte - CUPS Más Gestionado Anexo 9</h1>
                    ...
                    <tr>
                        <td class="tabForm">
                            ...
                            <form id="reportfm" name="reportfm" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get" onsubmit="return validateForm()">
                                ...
                                <input type="hidden" name="action" value="cup_a9" />
                                </br>&nbsp;
