...
/**
 * Project:		CMS_CALENDAR A general calendar class for
						date manipulation.
 * File:		csm_calendar.class.php
 *
...
class cms_calendar {
   ...
   function cms_calendar()
   {
	...
	function check_valid_date($date)
	{
	  ...
	  // Day Check
	  $days_in_month = $this->number_of_days_in_month($year, $month);
	...
	function add_delta_ymd($date, $delta_years = 0, $delta_months = 0, $delta_days = 0)
	{
	  ...
	  if ($delta_years != 0) {
		...
		if ($month == 2 && $day == 29) {
			...
			$days_in_month = $this->number_of_days_in_month($new_year, $new_month);
	...
	// Sunday before as part of the following ISO week.
	  if ($this->week_start == 0) { $day++; } // Add one to get to Monday.
	...
	function format_date_yyyymmdd_to_mmddyyyy($date, $del = '-') {
	  list($year, $month, $day) = explode($del,$date);
	  return $month.'/'.$day.'/'.$year;
	}
	
	function format_date_mmddyyyy_to_yyyymmdd($date, $del = '/') {
	  list($month, $day, $year) = explode($del,$date);
