...
{capture name="label" assign="label"}{sugar_translate label='LBL_GRUPO_DEL_USUARIO' module='Users'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.grupo_del_usuario_c.name}" 
id="{$fields.grupo_del_usuario_c.name}" 
title=''       
>
{if isset($fields.grupo_del_usuario_c.value) && $fields.grupo_del_usuario_c.value != ''}
{html_options options=$fields.grupo_del_usuario_c.options selected=$fields.grupo_del_usuario_c.value}
{else}
{html_options options=$fields.grupo_del_usuario_c.options selected=$fields.grupo_del_usuario_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.grupo_del_usuario_c.options }
{capture name="field_val"}{$fields.grupo_del_usuario_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.grupo_del_usuario_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.grupo_del_usuario_c.name}" 
id="{$fields.grupo_del_usuario_c.name}" 
title=''          
>
{if isset($fields.grupo_del_usuario_c.value) && $fields.grupo_del_usuario_c.value != ''}
...
<input
id="{$fields.grupo_del_usuario_c.name}-input"
name="{$fields.grupo_del_usuario_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.grupo_del_usuario_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.grupo_del_usuario_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.grupo_del_usuario_c.name}-input', '{$fields.grupo_del_usuario_c.name}');sync_{$fields.grupo_del_usuario_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.grupo_del_usuario_c.name}{literal}");
if (typeof select_defaults =="undefined")
select_defaults = [];
select_defaults[selectElem.id] = {key:selectElem.value,text:''};
//get default
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.grupo_del_usuario_c.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.grupo_del_usuario_c.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.grupo_del_usuario_c.name}');
{literal}
function SyncToHidden(selectme){
var selectElem = document.getElementById("{/literal}{$fields.grupo_del_usuario_c.name}{literal}");
var doSimulateChange = false;
...
//global variable 
sync_{/literal}{$fields.grupo_del_usuario_c.name}{literal} = function(){
SyncToHidden();
}
function syncFromHiddenToWidget(){
var selectElem = document.getElementById("{/literal}{$fields.grupo_del_usuario_c.name}{literal}");
...
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.simulate('keyup');
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.grupo_del_usuario_c.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.grupo_del_usuario_c.name}{literal}", syncFromHiddenToWidget);
{/literal}
SUGAR.AutoComplete.{$ac_key}.minQLen = 0;
SUGAR.AutoComplete.{$ac_key}.queryDelay = 0;
SUGAR.AutoComplete.{$ac_key}.numOptions = {$field_options|@count};
...
addToValidate('EditView', 'grupo_del_usuario_c', 'enum', false,'{/literal}{sugar_translate label='LBL_GRUPO_DEL_USUARIO' module='Users' for_js=true}{literal}' );
...
</script><script language="javascript">if(typeof sqs_objects == 'undefined'){var sqs_objects = new Array;}sqs_objects['EditView_reports_to_name']={"form":"EditView","method":"get_user_array","field_list":["user_name","id"],"populate_list":["reports_to_name","reports_to_id"],"required_list":["reports_to_id"],"conditions":[{"name":"user_name","op":"like_custom","end":"%","value":""}],"limit":"30","no_match_text":"Sin coincidencias"};</script>{/literal}
