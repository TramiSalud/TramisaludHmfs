...
class envio_de_correos_anexo_2{
    ...
    function envio_de_correos_anexo_2(){
		...
		$result = $GLOBALS['db']->query("SELECT a2.id AS ID_A2, a2.name as NO_A2, a2_cstm.numero_reintentos_c AS NO_REIN_A2C, 
										 ...
										 WHERE
											...
											AND a2_cstm.correo_enviado_c = 0
		...
		while ($row = $GLOBALS['db']->fetchByAssoc($result)){
			...
			{	
				...
				//file_put_contents('C:\\xampp\\htdocs\\tramisalud_dev_gbi\\log_envio_A2.txt',var_export($row['NO_A2'].' '.$time.' '.$now.' |A2|EPS|',true),FILE_APPEND);
				if($time >= 10)
				{						
					if($mail->Send())
					{	
