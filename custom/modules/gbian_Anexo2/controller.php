...
class gbian_Anexo2Controller extends SugarController{


	public function action_save()
	{
		$bean = $this->bean;
		$result = $bean->db->query("select count(*) as consecutivo from gbian_anexo2");
	...
	public function action_EnviarCorreo(){
		...
		$usuario = new Account();
		$usuario->retrieve($this->bean->gbian_anexo2_accountsaccounts_ida);
		
		$historial = new GBIHI_HistorialCorreo();
		$this->bean->load_relationship('gbian_anexo2_gbihi_historialcorreo_1');
		
		$emailObj = new Email(true);
		$defaults = $emailObj->getSystemDefaultEmail();
		$mail = new SugarPHPMailerExt();
		$mail->setMailerAlternative('hospitalizacion');
		$mail->Subject = $usuario->primernombre_c.' '.$usuario->segundonombre_c.' '.$usuario->primerapellido_c.' '.$usuario->segundoapellido_c.' '.$usuario->tipo_documento_c.' '.$usuario->name.' - ANEXO TECNICO NO 2 # '.$this->bean->name;
		...
		$mail->AddCustomHeader('X-Anexo: '.'AT2P');
		$mail->AddCustomHeader('X-Numero: '.'1');
		$mail->AddCustomHeader('X-AnexoID: '.$this->bean->id);
		$mail->AddAttachment("upload\PDF_Anexo2\A2-".$this->bean->name.".pdf", "ANEXO TECNICO NO 2 - Numero de Solicitud ".$this->bean->name.".pdf");
		...
		{	
			...
			{			
				...
				{	
					...
					$this->bean->gbian_anexo2_gbihi_historialcorreo_1->add($historial);
					$this->bean->save();
					
					?>
						<script language="JavaScript" type="text/javascript">
							alert("El correo fue enviado satisfactoriamente");
							window.location.href = window.location.protocol + "//" + window.location.host+'/tramisaludhmfs/index.php?module=gbian_Anexo2&action=DetailView&record=<?php echo $this->bean->id; ?>';				
						</script>
					<?php
				}
				else
