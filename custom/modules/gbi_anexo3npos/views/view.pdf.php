<?php
	...
	{		
		...
		function Header()
		{
			...
			$this->Cell(0,10,utf8_decode('ANEXO TÉCNICO No. 3 NO POS'),0,0,'C');
			$this->Ln(3);	
	...
	$anexo_3 =  BeanFactory::getBean("gbi_anexo3npos", $this->bean->id);
	...
	//$paciente =  BeanFactory::getBean("Accounts", $this->bean->account_id1_c);
	
	$pacientes = $this->bean->get_linked_beans('accounts_gbi_anexo3npos_1', 'Accounts');
	if (!empty($pacientes)){
		$paciente = null;
	...
	//$cups = $this->bean->get_linked_beans('minpo_Manejo_integra',$anexo_3->gbi_anexo3npos_minpo_manejo_integra_1gbi_anexo3npos_ida);
	$cups = $this->bean->get_linked_beans('gbi_anexo3npos_minpo_manejo_integra_1', 'minpo_Manejo_integra');
	$cup1=null;
	...
	if($anexo_3->origenatencion == 'enfermedad_g')
	{
		$pdf->SetXY(22,123);
		$pdf->Cell(0,10, 'X',0,0,'L');
	}
	
	if($anexo_3->origenatencion == 'enfermedad_p')
	{
		$pdf->SetXY(22,127);
		$pdf->Cell(0,10, 'X',0,0,'L');
	}
	
	if($anexo_3->origenatencion == 'Accidente_trab')
	{
		$pdf->SetXY(51,123);
		$pdf->Cell(0,10, 'X',0,0,'L');
	}
	
	if($anexo_3->origenatencion == 'accidente_trans')
	{
		$pdf->SetXY(51,127);
		$pdf->Cell(0,10, 'X',0,0,'L');
	}
	
	if($anexo_3->origenatencion == 'Evento')
	{
		$pdf->SetXY(81,123);
		$pdf->Cell(0,10, 'X',0,0,'L');
	}
	...
	if($anexo_3->tiposerviciosolicitado == 'posterior_urgencias')
	{
		$pdf->SetXY(112,123);
		$pdf->Cell(0,10, 'X',0,0,'L');
	}

	if($anexo_3->tiposerviciosolicitado == 'servicios_electivos')
	{
		$pdf->SetXY(112,127);
		$pdf->Cell(0,10, 'X',0,0,'L');
	}
	...
	$result = count($cups);
		...
		foreach ($cups as $item){
			...
			{	
				...
				$pdf->SetXY(50,$s);
				$pdf->Cell(0,10, $cup1->descripcion_cup_c,0,'L');
				
				$pdf->SetXY(27,$s);
				$pdf->Cell(0,10, $cup1->cantidad,0,0,'L');
	...
	$pdf->SetXY(55,246);
	$pdf->Cell(0,10,utf8_decode('Código CIE10'),0,0,'L');
	...
	$pdf->Output("ANEXO TECNICO NO 3 NO POS - Numero de Solicitud ".$anexo_3->name.".pdf",'I');	
?>