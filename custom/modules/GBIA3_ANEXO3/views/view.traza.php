<?php
	...
	$historial = $this->bean->get_linked_beans('gbia3_anexo3_gbihi_historialcorreo_1', 'GBIHI_HistorialCorreo');
	...
	$pdf->Cell(0,10, 'Informe: '.utf8_decode('Solicitud Autorización de Servicios de Salud '.$anexo_3->name),0,0,'L');
	$pdf->Ln(4);
	...
	$pdf->Ln(4);
	$pdf->Cell(0,10, utf8_decode('Fecha de petición de envío: ').utf8_decode($anexo_3->date_entered),0,0,'L');
	$pdf->Ln(4);
	...
	$correo_enviado = '';
	if($anexo_3->correo_enviado_c == 0) { $correo_enviado = 'NO'; }
	if($anexo_3->correo_enviado_c == 1) { $correo_enviado = 'SI'; }
	
	$marcado_enviar = '';
	if($anexo_3->marcado_enviar_c == 0) { $marcado_enviar = 'NO'; }
	if($anexo_3->marcado_enviar_c == 1) { $marcado_enviar = 'SI'; }
	
	$rebotado = '';
	if($anexo_3->rebotado_c == 0) { $rebotado = 'NO'; }
	if($anexo_3->rebotado_c == 1) { $rebotado = 'SI'; }
		
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(0,10, utf8_decode('Fecha Envío de Correo: ').$anexo_3->fecha_envio_correo_c,0,0,'L');
	$pdf->Ln(4);
	$pdf->Cell(0,10, 'Numero Reintentos: '.$anexo_3->numero_reintentos_c,0,0,'L');
	$pdf->Ln(4);
	...
	$pdf->Output("TRAZABILIDAD - AT3 - ".$anexo_3->name.".pdf",'I');	
?>