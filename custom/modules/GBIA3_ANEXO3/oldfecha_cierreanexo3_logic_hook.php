...
static $running = false;
	...
	function guardar($bean, $event, $arguments){
			...
			while ($contar=$db->fetchByAssoc($result))
			{
				if ($contar['total']==0)
				{
						$bean->load_relationship('gbia3_anexo3_tasks_1');
						$dt=new DateTime($bean->date_entered);
						$fecha=$dt->format('Y-m-d');
						$nuevafecha = strtotime ( '+7 DAY',strtotime ( $fecha ) ) ;
						$nuevafecha = date ('Y-m-d 23:00:00' , $nuevafecha );
						
						$tareas->name="Fecha de Cierre del Anexo 3 - ".$bean->name;
						...
						$bean->gbia3_anexo3_tasks_1->add($tareas);	
				}
			}
			...
			while ($tareasre=$db->fetchByAssoc($result2))
			{
				...
				if ($tareasre['name']=="Fecha de Cierre del Anexo 3 - ".$bean->name)
				{
